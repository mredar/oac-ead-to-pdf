...
<xsl:template match="language[@langcode]" mode="ead">
...
<xsl:variable name="hackedLink" select="
        replace(replace(@href,'http://ark.cdlib.org/ark:/', concat('/' , 'ark:/') ) ,'/$','')" />
...
<div class="online">
	<div class="left">
	<img width="41" height="20" title="Online items available" alt="Online items available" 
		src="./images/sq-eye_icon.gif" class="eye-icon" />   
	</div>
	<div class="right">
...
<xsl:template match="daogrp[@poi]" mode="ead-dsc" >
	<xsl:variable name="href">
		<xsl:text>/</xsl:text>
		<xsl:value-of select="@poi"/>
		<xsl:text>/?brand=oac4</xsl:text>
